...
const PageRenderer = ({ pageKey, componentName, ...props }) => {
  // If componentName is provided, render just that component
  if (componentName) {
    return <DynamicComponent componentName={componentName} {...props} />;
  }

  const config = useConfig();
  // Default to 'home' if no pageKey is provided
  const currentPage = pageKey || 'home';
  
  // Safely access pages from config
    ...
    return (
      <div className="p-4 border border-red-200 bg-red-50 rounded-md">
        ...
        <p className="text-sm text-red-600">
          The requested page "{currentPage}" does not exist. Available pages: {Object.keys(pages).join(', ')}
        </p>
      </div>
    );
  }
